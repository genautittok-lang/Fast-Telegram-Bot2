...
- **Enterprise tier fix v2**: Admin tier assignment (bot + web) now properly sets `requestsLeft` along with tier. `updateUserTier` in storage sets limits: FREE=5, PRO=50, ENTERPRISE/GROUPS=9999. Previously only changed the tier string without updating requests.

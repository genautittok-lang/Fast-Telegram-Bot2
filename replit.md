...
- **Ad Banner System v2**: Admin-managed promotional banners displayed as floating right-side ads on dashboard. `ds_ad_banners` table with title/description/imageUrl/mediaType/linkUrl/linkText/bgGradient/priority/showForTiers. Admin panel "Банери" tab with **file upload** (images + videos up to 50MB via multer) instead of URL input — drag-and-drop upload zone with preview. Dashboard shows banners as fixed bottom-right floating cards with: media display (auto-playing muted video or image), gradient overlay, "AD" badge, close button (X) with rotate animation, CTA button with gradient. Dismiss state persisted in localStorage. Max 2 banners shown at once. Mobile-responsive positioning. Upload endpoint: `POST /api/admin/banners/upload`. Public `GET /api/banners` + admin CRUD endpoints.
